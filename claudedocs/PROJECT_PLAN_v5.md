...
**完了:**
- ✅ **NDI出力実装**
  - NDI SDK動的ロード問題を解決（`DYLD_LIBRARY_PATH`設定）
  - 黒画面問題を解決（UYVY→NV12→UYVY変換で修正）
  - NDI出力が正常に動作することを確認

- ✅ **シークバー13秒オフセット問題の調査**
  - 原因特定: `async=false`がパイプラインのクロック同期を破壊
  - `pipeline.query_position()`が不正確な値を返す
  
- ✅ **ライブパイプラインアーキテクチャ設計**
  - TECHNICAL_DESIGN.md にセクション4.4「ライブパイプラインアーキテクチャ」を追加
  - 設計方針:
    - `pipeline.set_latency()` でライブモード有効化
    - `async=true` を維持してクロック同期を保持
    - `ts-offset` で出力先ごとの遅延調整（±300ms対応）
    - `queue.leaky=downstream` でライブ動作
  - 将来の要件対応:
    - NDI/Syphon/Spout/Display混在出力
    - ソース・出力先ごとの動的遅延調整
    - オーディオ/ビデオ別遅延設定
...
- 🔄 ライブパイプラインアーキテクチャの実装
  - `async=false` を削除
  - `pipeline.set_latency()` を追加
  - `position()` 関数をシンプルなパイプラインクエリに戻す

**技術ハイライト:**
- GStreamer `async` プロパティ: `false`にするとシンクがクロック同期に参加しなくなる
- GStreamer `sync` プロパティ: タイムスタンプベースの同期を有効化
- `pipeline.set_latency()`: ライブパイプラインモードを有効化し、全シンクに基準遅延を設定
- `ts-offset`: シンクごとの遅延調整（ナノ秒単位、動的変更可能）
