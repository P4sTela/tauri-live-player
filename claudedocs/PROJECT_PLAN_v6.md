...
│                                                             │
│  filesrc(side.mp4)  → decode → videobalance[side] → appsink│
│                                     ↓                       │
│                              Rust NdiSender (SDK直接)       │
└─────────────────────────────────────────────────────────────┘
```

**重要**: NDI出力は `ndisink` ではなく `appsink` + NDI SDK 直接呼び出しを使用。
これにより、ライブ/非ライブ混在問題（13秒オフセット等）を回避し、
正確な再生位置（PTS）を取得できる。
...
#### 機能
- NDI送信（`appsink` + NDI SDK 直接）★ ndisink は使わない
- NDI受信（`ndisrc`）
- ソース自動発見
...
```
送信: filesrc → decodebin → videobalance → appsink
                                              ↓
                                      Rust NdiSender (SDK直接)
                                              ↓
                                         NDI送信

受信: ndisrc → ndisrcdemux → videoconvert → autovideosink
                           → audioconvert → autoaudiosink
```

#### 実装内容
- NDI SDK の Rust FFI バインディング作成
- `NdiSender` 構造体（appsinkコールバック + SDK送信）
- `NdiReceiver` 構造体（ndisrc ラッパー）
- ソース発見（`NDIlib_find_*` API）
